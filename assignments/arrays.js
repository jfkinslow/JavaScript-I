...
console.log(`Car 33 is a %d %s %s`, inventory[32].car_year, inventory[32].car_make, inventory[32].car_model)

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = inventory.length - 1;
console.log("The last car is a %s %s", inventory[lastCar].car_make, inventory[lastCar].car_model);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = inventory;
carModels.sort((a, b) => {
    var makeA=a.car_make.toLowerCase(), modelA = a.car_model.toLowerCase(), makeB = b.car_make.toLowerCase(), modelB = b.car_model.toLowerCase();
    if (makeA < makeB)
        return -1;
    if (makeA > makeB)
        return 1;
...
let carYears = inventory.map((current, index, array) => {
    return current.car_year;
});
// carYears.sort();
...
let oldCars = carYears.map((current, index, arr) => {
    if (current < 2000) return current;
});
console.log(oldCars.length); 

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = inventory.map((current, index, arr) => {return (current.car_make == "Audi" || current.car_make == "BMW") ? current : undefined });
let filtered = BMWAndAudi.filter((e) => {
    return e != null;
});
